...
mock.onPost('/jwt/login').reply(async request => {
    ...
    try {
        ...
        let promiceA = (myTaskAPI.taskLogin(email, password))

        let result = await Promise.resolve(promiceA);

        let user = data.users.find(u => u.email === 'admin@demo.com' && u.password === 'admin');
        console.log(result.data.data[0].givenName)
        user.fullName = result.data.data[0].givenName;
        user.id = result.data.data[0].id;
