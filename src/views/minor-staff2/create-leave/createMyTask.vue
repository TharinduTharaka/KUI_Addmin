...
export default {
  ...
  data() {
    return {
      ...
      title: '',
      label: '',
      emp_name: localStorage.getItem('child_name').replaceAll("\"",""),
      taskDescription: '',
      reporterID: '',
      estimate: '0',
      originalEstimate: '',
  ...
  methods: {
    ...
    async getAllAvailableLeaves() {
      ...
      let response = (await leaveAPI.getLeaveBalance(localStorage.getItem('child_id')))
    ...
    async submit() {
      ...
      await leaveAPI.create(localStorage.getItem('child_id'),payload)
          .then((response) => {
