<template>
  <b-card-code title="Basic Salary">
    ...
    <b-button
        variant="secondary"
        v-b-modal.update-master-category
    >
      Add New Basic Category
    </b-button>
    <!-- input search -->
    ...
    <b-modal id="salary_info" title="Salary Breakdown" cancel-variant="outline-secondary" size="xl">
      <vue-good-table
...
export default {
  ...
  async mounted() {
    ...
    await this.getBasicSalaryInfoById(localStorage.getItem('child_id'))
  },
  ...
  methods: {
    ...
    async getBasicSalaryInfoById(id) {
      let response = (await payroll.getBasicSalaryById(id))
      ...
      await payroll.updateSalary(localStorage.getItem('child_id'), 'Basic Salary',this.selected,this.salaryAmount,userData.id, this.selected_for_Addition_type,this.reason)
          .then((res) => {
            ...
            if (res.data.code === 200){
              this.getBasicSalaryInfoById(localStorage.getItem('child_id'))
    ...
    async updateMasterSalaryType() {
      ...
      const userData = getUserData()
      await payroll.updateSalaryMasterDataType('Basic Salary',this.type,userData.id)
          .then((res) => {
            this.type = ''
    ...
    async getSalaryMasterDataByCategory() {
      ...
      await payroll.getSalaryMasterDataTypeByCategory('Basic Salary')
          .then((res) => {
