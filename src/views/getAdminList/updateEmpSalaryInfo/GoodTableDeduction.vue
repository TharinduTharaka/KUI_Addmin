<template>
  <b-card-code title="Allowances">
    ...
    <b-button
        variant="secondary"
        v-b-modal.update-master-category-deduction
    >
    ...
    <b-modal
        id="update-master-category-deduction"
    ...
    <b-modal id="salary_info_deduction" title="Salary Breakdown" cancel-variant="outline-secondary" size="xl">
      <vue-good-table
...
export default {
  ...
  async mounted() {
    ...
    await this.getDeductionSalaryById(localStorage.getItem('child_id'))
  },
  ...
  methods: {
    ...
    async getDeductionSalaryById(id) {
      let response = (await payroll.getDeductionSalaryById(id))
    ...
    async updateSalary() {
      ...
      await payroll.updateSalary(localStorage.getItem('child_id'), 'Deductions',this.selected,this.salaryAmount,userData.id, this.selected_for_Addition_type,this.reason, this.startDate)
          .then((res) => {
            this.selected = ''
